...
const ProductData2 = React.lazy(() => import('../productData/ProductData2'));
...
const Details2 = () => {
    ...
    async function getProducts() {
        try {
            const res = await axios.get('https://dummyjson.com/products/category/mens-watches?limit=1&skip=1');
                ...
                <Col sm={5} className='ps-0'>
                    <Card id='details1'>
                        {products.map((product) => (
                            <div key={product.id}>
                                <img className='thumbnails' src={product.thumbnail} alt="" />
                                {product.images.map((img, index) => (
                                    <Button variant="transparent" key={index}>
                                        <img className='zoom' src={img} alt="" style={{ width: "150px", marginTop: "20px" }} />
                                    </Button>
                                ))}
                            </div>
                        ))}
